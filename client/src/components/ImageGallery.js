    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data.resources.length > 0) {
          ...
          const pastWeatherResponse = await axios.get(`http://localhost:3001/api/weather/${latitude}/${longitude}/${date}`);
          ...
          const currentWeatherResponse = await axios.get(`http://localhost:3001/api/currentweather/${latitude}/${longitude}`);
