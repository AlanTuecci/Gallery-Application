  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`https://${URL}/gallery/api/images/${id}`);
        ...
        if (response.data.resources.length > 0) {
          ...
          const pastWeatherResponse = await axios.get(
            `https://${URL}/gallery/api/weather/${latitude}/${longitude}/${date}`
          );
          ...
          const currentWeatherResponse = await axios.get(
            `https://${URL}/gallery/api/currentweather/${latitude}/${longitude}`
          );
